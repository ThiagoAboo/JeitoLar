...
export default function ServiceGroupPage({ groupKey }) {
  ...
  const schema = {
    ...
    provider: {
      "@type": "Organization",
      "@id": "https://jeitolight-connector.lovable.app/#organization",
      name: "JeitoLar",
    },
    areaServed: regioes.map((item) => ({ "@type": "AdministrativeArea", name: item.nome })),
    url: `https://jeitolight-connector.lovable.app${group.path}`,
  };
      ...
      <section className="service-group-hero">
        <div className="container service-group-hero-grid">
          ...
          <div className="region-price-box">
            ...
            <h2>
              {region ? `Região selecionada: ${region.nome}` : "Selecione onde será o atendimento"}
            </h2>
            <p>
              A região escolhida no topo do site é reaproveitada em serviços e orçamento. Você pode
              alterá-la aqui a qualquer momento.
            </p>
            <label>
              Região
      ...
      <section className="section soft">
        <div className="container">
          <div className="section-title left">
            ...
            <p>
              {region
                ? `Valores de mão de obra para atendimento em ${region.nome}. O deslocamento é calculado na página de orçamento.`
                : "Escolha a região para liberar os valores de referência."}
            </p>
          </div>
          ...
          <div className="group-service-list">
            {services.map((service) => {
              ...
              const quoteSearch = regionId
                ? { regiao: regionId, servico: service.id }
                : { servico: service.id };
              return (
                <article className="group-service-row" key={service.id}>
                  <div>
                    <h3>{service.nome}</h3>
                  ...
                  <div className="group-service-price">
                    ...
                    {regionId ? (
                      ...
                      <span className="muted-action">Selecione a região acima</span>
                    )}
                  </div>
                </article>
              );
          ...
          <div className="catalog-cta">
            <div>
              <span className="eyebrow">Precisa de mais de um serviço?</span>
              <h2>Monte tudo em uma única estimativa.</h2>
              <p>
                Na calculadora você pode combinar serviços de diferentes grupos, quantidades e
                detalhes do atendimento.
              </p>
            </div>
            <Link className="btn" to="/orcamento" search={regionId ? { regiao: regionId } : {}}>
