...
const groupedServices = servicos.reduce((acc, s) => { (acc[s.categoria] ||= []).push(s); return acc; }, {});
...
export default function QuoteBuilder() {
  ...
  const filteredGroups = useMemo(() => {
    ...
    const out = {};
    for (const [cat, list] of Object.entries(groupedServices)) { const filtered = list.filter((s) => `${s.nome} ${s.descricao} ${s.categoria}`.toLowerCase().includes(q)); if (filtered.length) out[cat] = filtered; }
    return out;
  }, [query]);

  const details = useMemo(() => items.map((item) => {
    const service = serviceById[item.serviceId];
    const price = regionId ? serviceLineTotal(item.serviceId, regionId, item.quantity, item.extras) : { total:0, unitBase:0, additionalUnit:0, extrasTotal:0 };
  ...
  return <div className="quote-shell">
    <div className="quote-main">
      <section className="quote-step"><div className="step-number">1</div><div className="step-heading"><h2>Onde será o atendimento?</h2><p>A localidade ajuda a calcular os valores de referência e o deslocamento estimado.</p></div>
        <div className="form-grid"><label>Região<select ref={regionRef} className={validationField === "region" ? "field-error" : ""} value={regionId} onChange={(e)=>{setRegionId(e.target.value); setValidationField("");}}><option value="">Selecione...</option>{regioes.map((r)=><option value={r.id} key={r.id}>{r.nome}</option>)}</select></label>
        <label>Bairro<select ref={neighborhoodRef} className={validationField === "neighborhood" ? "field-error" : ""} value={neighborhoodId} onChange={(e)=>{setNeighborhoodId(e.target.value); setValidationField("");}} disabled={!regionId}><option value="">Selecione...</option>{neighborhoods.map((b)=><option value={b.id} key={b.id}>{b.nome}</option>)}</select></label></div>
        {neighborhoodId === "outro" && <label className="full-label">Informe o bairro<input ref={neighborhoodOtherRef} className={validationField === "neighborhoodOther" ? "field-error" : ""} value={neighborhoodOther} onChange={(e)=>{setNeighborhoodOther(e.target.value); setValidationField("");}} placeholder="Digite seu bairro" /></label>}
        <label className="full-label">Endereço ou referência <span>(opcional nesta etapa)</span><input value={address} onChange={(e)=>setAddress(e.target.value)} placeholder="Rua, número, condomínio ou ponto de referência" /></label>
      </section>

      <section ref={servicesStepRef} className={`quote-step ${validationField === "services" ? "step-error" : ""}`}><div className="step-number">2</div><div className="step-heading"><h2>O que você precisa?</h2><p>Adicione serviços, quantidades e detalhes. Os valores aparecem depois da seleção da localidade.</p></div>
        <input ref={serviceSearchRef} className="service-search" value={query} onChange={(e)=>{setQuery(e.target.value); if (validationField === "services") setValidationField("");}} placeholder="Buscar: chuveiro, TV, torneira, roçagem, jardim, piscina..." />
        {!regionId && <div className="notice">Selecione a região primeiro para visualizar os valores de referência.</div>}
        {Object.entries(filteredGroups).map(([category,list]) => {
          ...
          return <div className={`service-group accordion ${expanded ? "open" : ""}`} key={category}>
            <button
              type="button"
            ...
            >
              ...
              <span className="service-group-status">
                {selectedCount ? `${selectedCount} selecionado${selectedCount > 1 ? "s" : ""}` : `${list.length} opções`}
      ...
      <section className="quote-step"><div className="step-number">3</div><div className="step-heading"><h2>Quem está solicitando?</h2><p>Seu telefone não é necessário aqui: o pedido será enviado pelo seu próprio WhatsApp.</p></div>
        ...
        <label className="full-label">Observações <span>(opcional)</span><textarea value={notes} onChange={(e)=>setNotes(e.target.value)} rows="4" placeholder="Ex.: tenho o suporte da TV; parede de alvenaria; preciso realizar tudo no mesmo dia..." /></label>
      </section>
    </div>
